...
import { Plus, ArrowUpCircle, CheckCircle2, Loader2, Search, X, AlertTriangle } from "lucide-react";
...
export default function ContasReceberPage() {
  ...
  function carregar() {
    ...
    fetch(`/api/contas-receber/list?${params}`)
      .then((r) => r.ok && r.json())
      .then((res) => {
        setContas(res.data || []);
  ...
  useEffect(() => { carregar(); }, [page, busca, filtroStatus, filtroPeriodo, dataInicio, dataFim]);
  ...
  useEffect(() => { setPage(1); }, [busca, filtroStatus, filtroPeriodo, dataInicio, dataFim]);

  const handleReceber = useCallback(async (id: string) => {
    await fetch(`/api/contas-receber/${id}`, { method: "PATCH" });
    carregar();
  }, []);
  ...
  const temFiltro = busca || filtroStatus || filtroPeriodo;
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              {temFiltro ? "Tente limpar os filtros" : "Cadastre contas a receber para controlar suas entradas"}
            </p>
            {temFiltro ? (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
                          ...
                          {conta.cliente && <span>{conta.cliente.nome}</span>}
                      ...
                      <div className="flex gap-2">
                        {conta.status !== "PAGO" && (
                          <Button variant="outline" size="sm" onClick={() => handleReceber(conta.id)}>
